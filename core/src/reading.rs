...
impl ReadingEngine {
    ...
    pub fn choose_symbol(
        ...
        // Get passage — deterministic for Hexagram, random for others
        let passage = match session.source.source_type {
            SourceType::Hexagram => {
                // Convention: symbol "hexagram_N" → passage "iching_N"
                let passage_id = symbol_id.replace("hexagram_", "iching_");
                self.store
                    .get_passage_by_id(&passage_id)?
                    .map(Ok)
                    .unwrap_or_else(|| {
                        warn!(
                            "Hexagram passage not found for symbol_id={}, falling back to random",
                            symbol_id
                        );
                        self.store
