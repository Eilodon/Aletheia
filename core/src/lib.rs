...
impl InterpretationStreamJob {
    fn new() -> Self {
        Self {
            chunks: Vec::new(), delivered_chunks: 0, full_text: String::new(),
            done: false, used_fallback: false, cancelled: false,
    ...
    fn pending_state(request_id: &str) -> InterpretationStreamState {
        InterpretationStreamState {
            ...
            new_chunks: Vec::new(), full_text: String::new(),
            done: false, used_fallback: false, cancelled: false, error: None,
        }
    }
}
...
impl AletheiaCore {
    ...
    pub fn new(db_path: String, gift_backend_url: String) -> Self {
        match Self::try_new(&db_path, &gift_backend_url) {
            ...
            Err(error) => {
                ...
                let dummy = Arc::new(
                    Store::new(":memory:").expect("Cannot create in-memory fallback DB"),
                );
    ...
    pub fn seed_bundled_data(
        ...
        if let Some(err) = &self.init_error {
            return SeedBundledDataResponse { seeded: false, error: Some(err.clone()) };
        }
        ...
        match result {
            Ok(seeded) => SeedBundledDataResponse { seeded, error: None },
            Err(e)     => SeedBundledDataResponse { seeded: false, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn bootstrap_bundled_content(&self) -> SeedBundledDataResponse {
        ...
        let result = (|| -> Result<bool, AletheiaError> {
            ...
            self.store.seed_bundled_data(&artifact.sources, &artifact.passages, &artifact.themes)
        })();
        match result {
            Ok(seeded) => SeedBundledDataResponse { seeded, error: None },
            Err(e)     => SeedBundledDataResponse { seeded: false, error: Some(BridgeError::from_aletheia_error(&e)) },
    ...
    pub fn perform_reading(
        ...
        if let Some(err) = &self.init_error {
            return PerformReadingResponse { session: None, error: Some(err.clone()) };
        }
        match self.reading_engine.perform_reading(&user_id, source_id, situation_text) {
            Ok(session) => PerformReadingResponse { session: Some(session), error: None },
            Err(e)      => PerformReadingResponse { session: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }

    pub fn choose_symbol(&self, session: ReadingSession, symbol_id: String, method: SymbolMethod) -> ChooseSymbolResponse {
        match self.reading_engine.choose_symbol(&session, &symbol_id, method) {
            Ok(chosen) => ChooseSymbolResponse { chosen: Some(chosen), error: None },
            Err(e)     => ChooseSymbolResponse { chosen: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn complete_reading(&self, user_id: String, reading: Reading) -> CompleteReadingResponse {
        match self.reading_engine.complete_reading(&user_id, reading) {
            Ok(completed) => CompleteReadingResponse { completed: Some(completed), error: None },
            Err(e)        => CompleteReadingResponse { completed: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn get_fallback_prompts(&self, source_id: String) -> FallbackPromptsResponse {
        match self.reading_engine.get_fallback_prompts(&source_id) {
            Ok(prompts) => FallbackPromptsResponse { prompts, error: None },
            Err(e)      => FallbackPromptsResponse { prompts: Vec::new(), error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn set_ai_api_key(&self, provider: String, key: String) -> SetApiKeyResponse {
        if let Some(err) = &self.init_error {
            return SetApiKeyResponse { applied: false, error: Some(err.clone()) };
        }
        let provider_enum = match provider.to_lowercase().as_str() {
            ...
            _ => return SetApiKeyResponse {
                applied: false,
                error: Some(BridgeError::from_aletheia_error(&AletheiaError::invalid_input("provider", "unknown"))),
            },
        };
        self.ai_client.lock().set_api_key(provider_enum, key);
        SetApiKeyResponse { applied: true, error: None }
    }
    ...
    pub fn start_interpretation_stream(
        ...
        self.interpretation_cancel_tokens.lock()
            .insert(request_id.clone(), Arc::clone(&cancel_token));

        self.interpretation_jobs.lock()
        ...
        std::thread::spawn(move || {
            ...
            let result = {
                ...
                RUNTIME.block_on(client.request_interpretation_with_callback(
                    &passage, &symbol, situation_text.as_deref(), user_intent.as_deref(),
                    Arc::clone(&cancel_clone), Some(on_chunk), use_sonnet,
                ))
            };
                ...
                match result {
                    Ok(interp) => {
                        if job.chunks.is_empty() {
                            for chunk in interp.chunks { job.full_text.push_str(&chunk); job.chunks.push(chunk); }
                        }
        ...
        StartInterpretationStreamResponse { request_id: Some(request_id), error: None }
    }
    ...
    pub fn cancel_interpretation_stream(&self, request_id: String) -> CancelInterpretationResponse {
        let cancelled = if let Some(token) = self.interpretation_cancel_tokens.lock().get(&request_id).cloned() {
        ...
        if let Some(job) = self.interpretation_jobs.lock().get_mut(&request_id) {
            ...
            if cancelled { job.done = true; job.completed_at = Some(Instant::now()); }
        }

        CancelInterpretationResponse { cancelled, error: None }
    }
    ...
    pub fn request_interpretation(
        ...
        let result = {
            ...
            RUNTIME.block_on(client.request_interpretation(
                &passage, &symbol, situation_text.as_deref(), None, cancel,
            ))
        };
        match result {
            Ok(i)  => RequestInterpretationResponse { interpretation: Some(i), error: None },
            Err(e) => RequestInterpretationResponse { interpretation: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn get_daily_notification_message(&self, user_id: String, date: String) -> NotificationMessageResponse {
        if let Some(err) = &self.init_error {
            return NotificationMessageResponse { message: None, error: Some(err.clone()) };
        }
        let result = (|| -> Result<NotificationMessage, AletheiaError> {
            let entry = self.notification_scheduler.get_daily_notification(&user_id, &date)?;
            let (title, body) = self.notification_scheduler.format_notification(&entry);
            Ok(NotificationMessage { symbol_id: entry.symbol_id, question: entry.question, title, body })
        })();
        match result {
            Ok(msg) => NotificationMessageResponse { message: Some(msg), error: None },
            Err(e)  => NotificationMessageResponse { message: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn get_readings(&self, limit: u32, offset: u32) -> PaginatedReadingsResponse {
        if let Some(err) = &self.init_error {
            return PaginatedReadingsResponse { readings: None, error: Some(err.clone()) };
        }
        let result = (|| -> Result<PaginatedReadings, AletheiaError> {
            ...
            Ok(PaginatedReadings { items, total_count, has_more })
        })();
        match result {
            Ok(r)  => PaginatedReadingsResponse { readings: Some(r), error: None },
            Err(e) => PaginatedReadingsResponse { readings: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }

    pub fn get_reading_by_id(&self, id: String) -> ReadingResponse {
        if let Some(err) = &self.init_error {
            return ReadingResponse { reading: None, error: Some(err.clone()) };
        }
        match self.store.get_reading_by_id(&id) {
            Ok(r)  => ReadingResponse { reading: r, error: None },
            Err(e) => ReadingResponse { reading: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }

    pub fn update_reading_flags(&self, id: String, is_favorite: Option<bool>, shared: Option<bool>) -> ReadingResponse {
        if let Some(err) = &self.init_error {
            return ReadingResponse { reading: None, error: Some(err.clone()) };
        }
        let result = (|| -> Result<Option<Reading>, AletheiaError> {
            let Some(mut reading) = self.store.get_reading_by_id(&id)? else { return Ok(None); };
            if let Some(v) = is_favorite { reading.is_favorite = v; }
            if let Some(v) = shared      { reading.shared = v; }
    ...
    pub fn get_sources(&self, premium_allowed: bool) -> SourcesResponse {
        if let Some(err) = &self.init_error {
            return SourcesResponse { sources: Vec::new(), error: Some(err.clone()) };
        }
        match self.store.get_sources(premium_allowed) {
            Ok(sources) => SourcesResponse { sources, error: None },
            Err(e)      => SourcesResponse { sources: Vec::new(), error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn get_user_state(&self, user_id: String) -> UserStateResponse {
        if let Some(err) = &self.init_error {
            return UserStateResponse { state: None, error: Some(err.clone()) };
        }
        match self.store.get_user_state(&user_id) {
            Ok(state) => UserStateResponse { state: Some(state), error: None },
            Err(e)    => UserStateResponse { state: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }

    pub fn update_user_state(&self, state: UserState) -> UpdateUserStateResponse {
        if let Some(err) = &self.init_error {
            return UpdateUserStateResponse { updated: false, error: Some(err.clone()) };
        }
        match self.store.update_user_state(&state) {
            Ok(())  => UpdateUserStateResponse { updated: true, error: None },
            Err(e)  => UpdateUserStateResponse { updated: false, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn redeem_gift(&self, token: String) -> RedeemGiftResponse {
        if let Some(err) = &self.init_error {
            return RedeemGiftResponse { gift: None, error: Some(err.clone()) };
        }
        match RUNTIME.block_on(self.gift_client.redeem_gift(&token)) {
            Ok(g)  => RedeemGiftResponse {
                gift: Some(GiftReadingData {
                    token: g.token, buyer_note: g.buyer_note, source_id: g.source_id,
                    created_at: g.created_at, expires_at: g.expires_at, redeemed: g.redeemed,
                }),
                error: None,
            },
            Err(e) => RedeemGiftResponse { gift: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }

    pub fn create_gift(&self, source_id: Option<String>, buyer_note: Option<String>) -> CreateGiftResponse {
        if let Some(err) = &self.init_error {
            return CreateGiftResponse { token: None, deep_link: None, error: Some(err.clone()) };
        }
        match RUNTIME.block_on(self.gift_client.create_gift(source_id, buyer_note)) {
            Ok(r)  => CreateGiftResponse { token: Some(r.token), deep_link: Some(r.deep_link), error: None },
            Err(e) => CreateGiftResponse { token: None, deep_link: None, error: Some(BridgeError::from_aletheia_error(&e)) },
        }
    }
    ...
    pub fn check_device_capability(&self) -> DeviceCapabilityResponse {
        DeviceCapabilityResponse {
            capability: Some(DeviceCapability {
                supported: false, available_ram_mb: 0, cpu_cores: 0,
                has_simd: false, estimated_tps: 0.0,
    ...
    pub fn get_local_model_status(&self) -> LocalModelStatusResponse {
        LocalModelStatusResponse { model_info: Some(LocalModelInfo::default()), error: None }
    }
    ...
    pub fn delete_local_model(&self) -> bool { false }
}
