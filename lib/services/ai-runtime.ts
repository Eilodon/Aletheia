...
class AIRuntimeService {
  ...
  async startInterpretationStream(request: AIRuntimeRequest): Promise<string | null> {
    ...
    // Check subscription tier for model selection
    let useSonnet = false;
    try {
      const userId = await import("@/lib/services/current-user-id").then(m => m.getCurrentUserId());
      const userStateResponse = await aletheiaNativeClient.getUserState(userId);
      useSonnet = userStateResponse.state?.subscription_tier === "pro";
    } catch {
      // Default to Haiku on any error
      useSonnet = false;
    }
